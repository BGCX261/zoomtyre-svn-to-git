...
class EArray {
	...
	/**
	 * Удаляет пустые елементы из массива
	 */
	public static function trimEmpty(array $array){
	...
	/**
	 * Опеределяет глубину массива
	 */
	
	public static function getDepth($array){
		if(is_array($array)){
			foreach($array as $e)
				return 1+CArray::getDepth($e);
		} else
			return 0;
	}
	
	/**
	 * Собирает все значения из многомерного массива не сохраняет ключи
	 */
	public static function collectValues($array, &$result = array()){
		if(!is_array($array)){
			$result[] = $array;
			return $array;
		} else
			foreach($array as $e)
				CArray::collectValues($e, $result);
